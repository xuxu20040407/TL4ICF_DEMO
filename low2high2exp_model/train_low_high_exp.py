...
RemoveDir(current_dir+r'\low_high_exp_logs')
...
train_dataset = MYDATA(np.load(r".\DATA\High\train50.npy"))
train_dataloader = DataLoader(train_dataset, batch_size=25, drop_last=True, shuffle
=True)
val_dataset = MYDATA(np.load(r".\DATA\High\val.npy"))
...
writer = SummaryWriter(current_dir+r"\low_high_exp_logs")
...
torch.save(model.state_dict(), current_dir+r'\low_high_model_TL.pth')
...
for epoch in range(epochs2):
    ...
    if epoch % 50 == 0:
        print('epoch:' + str(epoch) + '\nrunning_loss=' + str(running_loss))
    writer.add_scalar("running_loss", torch.log(running_loss), epoch+epochs)
    ...
    if epoch % 50 == 0:
        print('test_loss=' + str(test_loss))
    writer.add_scalar("test_loss", torch.log(test_loss), epoch+epochs)
    writer.flush()


torch.save(model.state_dict(), current_dir+r'\low_high_exp_model_TL.pth')
